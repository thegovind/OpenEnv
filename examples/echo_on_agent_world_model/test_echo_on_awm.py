...
def test_roles_present_and_ordered():
    ...
    assert ACTION in roles and ENV_OUTPUT in roles
    assert WARNING in roles                         # fixture step 2 has a warning
    # the warning must come from its own segment, not env_output text
...
def test_warning_excluded_by_default_included_on_all():
    ...
    assert int(all_obs["obs_mask"].sum()) == int(
        env_only["obs_mask"].sum()
    ) + int(env_only["warning_mask"].sum())
...
def test_loss_is_finite_and_differentiable():
    ...
    loss, _ = echo_loss(logits, input_ids, m["action_mask"], m["obs_mask"],
                        torch.tensor([1.0]), world_model_coeff=0.05)
